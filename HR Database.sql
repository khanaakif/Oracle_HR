...
<table summary="" border="0" cellspacing="4" cellpadding="0" width="100%">
  <tr valign="top"> 
    <td height="20">
      ...
      <h3 class="topstoryhead">load_sample.sql</h3>
      <!-- #EndEditable --></td>
    <td height="20">
  ...
  <tr valign="top"> 
    <td height="20" colspan="2"> <!-- #BeginEditable "text" -->
      <p class="bodycopy">The source code for the load_sample.sql is:</p>
      <pre>REM ********************************************************************
...
REM HR.LOCATIONS table has a foreign key to this table.
   ...
   , region_name VARCHAR2(25) 
      ...
      <pre>CREATE UNIQUE INDEX reg_id_pk
         ON regions (region_id);</pre>
      <pre>ALTER TABLE regions
...
REM OE.CUSTOMERS table and HR.LOCATIONS have a foreign key to this table.
   ...
   ORGANIZATION INDEX; </pre>
      <pre>ALTER TABLE countries
...
REM HR.DEPARTMENTS has a foreign key to this table.
   ...
   , country_id CHAR(2)
      ...
      <pre>CREATE UNIQUE INDEX loc_id_pk
         ON locations (location_id) ;</pre>
      <pre>ALTER TABLE locations
   ...
   REFERENCES countries(country_id) 
      ...
      <pre>Rem Useful for any subsequent addition of rows to locations table
Rem Starts with 3300</pre>
      <pre>CREATE SEQUENCE locations_seq
   START WITH 3300
   INCREMENT BY 100
   MAXVALUE 9900
   NOCACHE
...
REM HR.EMPLOYEES and HR.JOB_HISTORY have a foreign key to this table.
   ...
   , location_id NUMBER(4)
      ...
      <pre>CREATE UNIQUE INDEX dept_id_pk
         ON departments (department_id) ;</pre>
   ...
   REFERENCES locations (location_id)
      ...
      <pre>Rem Useful for any subsequent addition of rows to departments table
Rem Starts with 280 </pre>
      <pre>CREATE SEQUENCE departments_seq
   START WITH 280
   INCREMENT BY 10
   MAXVALUE 9990
   NOCACHE
...
REM HR.EMPLOYEES has a foreign key to this table.
   ...
   , max_salary NUMBER(6)
      ...
      <pre>CREATE UNIQUE INDEX job_id_pk 
         ON jobs (job_id) ;</pre>
      <pre>ALTER TABLE jobs
...
REM HR.EMPLOYEES has a self referencing foreign key to this table.
   ...
   UNIQUE (email)
      ...
      <pre>CREATE UNIQUE INDEX emp_emp_id_pk
   ...
   REFERENCES employees (employee_id)
      ...
      <pre>Rem Useful for any subsequent addition of rows to employees table
REM Starts with 207 
       </pre>
      <pre>CREATE SEQUENCE employees_seq
   START WITH 207
   INCREMENT BY 1
   NOCACHE
   NOCYCLE;</pre>
...
REM HR.JOBS, HR_DEPARTMENTS, and HR.EMPLOYEES have a foreign key to this table.
   ...
   CHECK (end_date &gt; start_date)
      ...
      <pre>CREATE UNIQUE INDEX jhist_emp_id_st_date_pk 
         ON job_history (employee_id, start_date) ;</pre>
      <pre>ALTER TABLE job_history
...
REM about employees.
      ...
      <pre>CREATE OR REPLACE VIEW emp_details_view
   (employee_id,
   job_id,
   manager_id,
   department_id,
   ...
   WITH READ ONLY;</pre>
      <pre>&nbsp;</pre>
      <pre>COMMIT;</pre>
      <pre>ALTER SESSION SET NLS_LANGUAGE=American; </pre>
      <pre>REM ***************************insert data into the REGIONS table</pre>
      <pre>INSERT INTO regions VALUES 
   ( 1
   , 'Europe' 
   );</pre>
      <pre>INSERT INTO regions VALUES 
   ...
   , 'Middle East and Africa' 
      ...
      <pre>REM ***************************insert data into the COUNTRIES table</pre>
      <pre>INSERT INTO countries VALUES 
   ( 'IT'
   , 'Italy'
   , 1 
   );</pre>
   ...
   , 1 
      ...
      <pre>REM ***************************insert data into the LOCATIONS table       </pre>
      <pre>INSERT INTO locations VALUES 
   ( 1000 
   ...
   , 'MX'
      ...
      <pre>REM ****************************insert data into the DEPARTMENTS table</pre>
      <pre>REM disable integrity constraint to EMPLOYEES to load data</pre>
      <pre>ALTER TABLE departments 
   DISABLE CONSTRAINT dept_mgr_fk;</pre>
      <pre>INSERT INTO departments VALUES 
   ( 10
   , 'Administration'
   , 200
   , 1700
   ...
   , 1700
      ...
      <pre>REM ***************************insert data into the JOBS table</pre>
   ...
   , 10500
      ...
      <pre>REM ***************************insert data into the EMPLOYEES table</pre>
   ...
   , 110
      ...
      <pre>REM ********* insert data into the JOB_HISTORY table
       </pre>
      <pre>INSERT INTO job_history
         VALUES (102
   , TO_DATE('13-JAN-1993', 'dd-MON-yyyy')
   ...
   , 90
      ...
      <pre>REM enable integrity constraint to DEPARTMENTS</pre>
      <pre>ALTER TABLE departments 
   ENABLE CONSTRAINT dept_mgr_fk;</pre>
      <pre>COMMIT;</pre>
      <pre>CREATE INDEX emp_department_ix
   ON employees (department_id);</pre>
      <pre>CREATE INDEX emp_job_ix
   ON employees (job_id);</pre>
      <pre>CREATE INDEX emp_manager_ix
   ON employees (manager_id);</pre>
      <pre>CREATE INDEX emp_name_ix
   ON employees (last_name, first_name);</pre>
      <pre>CREATE INDEX dept_location_ix
   ON departments (location_id);</pre>
      <pre>CREATE INDEX jhist_job_ix
   ON job_history (job_id);</pre>
      <pre>CREATE INDEX jhist_employee_ix
   ON job_history (employee_id);</pre>
      <pre>CREATE INDEX jhist_department_ix
   ON job_history (department_id);</pre>
      <pre>CREATE INDEX loc_city_ix
   ON locations (city);</pre>
      <pre>CREATE INDEX loc_state_province_ix 
   ON locations (state_province);</pre>
      <pre>CREATE INDEX loc_country_ix
   ON locations (country_id);</pre>
      <pre>COMMIT;</pre>
      <pre>REM procedure and statement trigger to allow dmls during business hours:
   ...
   END secure_dml;
   /</pre>
      <pre>CREATE OR REPLACE TRIGGER secure_employees
   BEFORE INSERT OR UPDATE OR DELETE ON employees
   BEGIN
   secure_dml;
   END secure_employees;
   /</pre>
      <pre>ALTER TRIGGER secure_employees DISABLE;</pre>
      <pre>REM **************************************************************************
...
REM department_id columns in the EMPLOYEES table:</pre>
      <pre>CREATE OR REPLACE PROCEDURE add_job_history
   ...
   END add_job_history;
   /</pre>
      <pre>CREATE OR REPLACE TRIGGER update_job_history
   ...
   /</pre>
      <pre>COMMIT;</pre>
      <pre>COMMENT ON TABLE regions 
         IS 'Regions table that contains region numbers and names. Contains 4 rows; references with the Countries table.';</pre>
      <pre>COMMENT ON COLUMN regions.region_id
         IS 'Primary key of regions table.';</pre>
      <pre>COMMENT ON COLUMN regions.region_name
         IS 'Names of regions. Locations are in the countries of these regions.';</pre>
      <pre>COMMENT ON TABLE locations
         IS 'Locations table that contains specific address of a specific office,
         warehouse, and/or production site of a company. Does not store addresses /
         locations of customers. Contains 23 rows; references with the
         departments and countries tables. ';</pre>
      <pre>COMMENT ON COLUMN locations.location_id
         IS 'Primary key of locations table';</pre>
      <pre>COMMENT ON COLUMN locations.street_address
         IS 'Street address of an office, warehouse, or production site of a company.
         Contains building number and street name';</pre>
      <pre>COMMENT ON COLUMN locations.postal_code
         IS 'Postal code of the location of an office, warehouse, or production site 
         of a company. ';</pre>
      <pre>COMMENT ON COLUMN locations.city
         IS 'A not null column that shows city where an office, warehouse, or 
         production site of a company is located. ';</pre>
      <pre>COMMENT ON COLUMN locations.state_province
         IS 'State or Province where an office, warehouse, or production site of a 
         company is located.';</pre>
      <pre>COMMENT ON COLUMN locations.country_id
         IS 'Country where an office, warehouse, or production site of a company is
         located. Foreign key to country_id column of the countries table.';
       </pre>
      <pre>REM *********************************************</pre>
      <pre>COMMENT ON TABLE departments
         IS 'Departments table that shows details of departments where employees 
         work. Contains 27 rows; references with locations, employees, and job_history tables.';</pre>
      <pre>COMMENT ON COLUMN departments.department_id
         IS 'Primary key column of departments table.';</pre>
      <pre>COMMENT ON COLUMN departments.department_name
         IS 'A not null column that shows name of a department. Administration, 
         Marketing, Purchasing, Human Resources, Shipping, IT, Executive, Public 
         Relations, Sales, Finance, and Accounting. ';</pre>
      <pre>COMMENT ON COLUMN departments.manager_id
         IS 'Manager_id of a department. Foreign key to employee_id column of employees table. The manager_id column of the employee table references this column.';</pre>
      <pre>COMMENT ON COLUMN departments.location_id
         IS 'Location id where a department is located. Foreign key to location_id column of locations table.';
       </pre>
      <pre>REM *********************************************</pre>
      <pre>COMMENT ON TABLE job_history
         IS 'Table that stores job history of the employees. If an employee 
         changes departments within the job or changes jobs within the department, 
         new rows get inserted into this table with old job information of the 
         employee. Contains a complex primary key: employee_id+start_date.
         Contains 25 rows. References with jobs, employees, and departments tables.';</pre>
      <pre>COMMENT ON COLUMN job_history.employee_id
         IS 'A not null column in the complex primary key employee_id+start_date.
         Foreign key to employee_id column of the employee table';</pre>
      <pre>COMMENT ON COLUMN job_history.start_date
         IS 'A not null column in the complex primary key employee_id+start_date. 
         Must be less than the end_date of the job_history table. (enforced by 
         constraint jhist_date_interval)';</pre>
      <pre>COMMENT ON COLUMN job_history.end_date
         IS 'Last day of the employee in this job role. A not null column. Must be 
         greater than the start_date of the job_history table. 
         (enforced by constraint jhist_date_interval)';</pre>
      <pre>COMMENT ON COLUMN job_history.job_id
         IS 'Job role in which the employee worked in the past; foreign key to 
         job_id column in the jobs table. A not null column.';</pre>
      <pre>COMMENT ON COLUMN job_history.department_id
         IS 'Department id in which the employee worked in the past; foreign key to deparment_id column in the departments table';
       </pre>
      <pre>REM *********************************************</pre>
      <pre>COMMENT ON TABLE countries
         IS 'country table. Contains 25 rows. References with locations table.';</pre>
      <pre>COMMENT ON COLUMN countries.country_id
         IS 'Primary key of countries table.';</pre>
      <pre>COMMENT ON COLUMN countries.country_name
         IS 'Country name';</pre>
      <pre>COMMENT ON COLUMN countries.region_id
         IS 'Region ID for the country. Foreign key to region_id column in the departments table.';</pre>
      <pre>REM *********************************************</pre>
      <pre>COMMENT ON TABLE jobs
         IS 'jobs table with job titles and salary ranges. Contains 19 rows.
         References with employees and job_history table.';</pre>
      <pre>COMMENT ON COLUMN jobs.job_id
         IS 'Primary key of jobs table.';</pre>
      <pre>COMMENT ON COLUMN jobs.job_title
         IS 'A not null column that shows job title, e.g. AD_VP, FI_ACCOUNTANT';</pre>
      <pre>COMMENT ON COLUMN jobs.min_salary
         IS 'Minimum salary for a job title.';</pre>
      <pre>COMMENT ON COLUMN jobs.max_salary
         IS 'Maximum salary for a job title';</pre>
      <pre>REM *********************************************</pre>
      <pre>COMMENT ON TABLE employees
         IS 'employees table. Contains 107 rows. References with departments, 
         jobs, job_history tables. Contains a self reference.';</pre>
      <pre>COMMENT ON COLUMN employees.employee_id
         IS 'Primary key of employees table.';</pre>
      <pre>COMMENT ON COLUMN employees.first_name
         IS 'First name of the employee. A not null column.';</pre>
      <pre>COMMENT ON COLUMN employees.last_name
         IS 'Last name of the employee. A not null column.';</pre>
      <pre>COMMENT ON COLUMN employees.email
         IS 'Email id of the employee';</pre>
      <pre>COMMENT ON COLUMN employees.phone_number
         IS 'Phone number of the employee; includes country code and area code';</pre>
      <pre>COMMENT ON COLUMN employees.hire_date
         IS 'Date when the employee started on this job. A not null column.';</pre>
      <pre>COMMENT ON COLUMN employees.job_id
         IS 'Current job of the employee; foreign key to job_id column of the 
         jobs table. A not null column.';</pre>
      <pre>COMMENT ON COLUMN employees.salary
         IS 'Monthly salary of the employee. Must be greater 
         than zero (enforced by constraint emp_salary_min)';</pre>
      <pre>COMMENT ON COLUMN employees.commission_pct
         IS 'Commission percentage of the employee; Only employees in sales 
         department elgible for commission percentage';</pre>
      <pre>COMMENT ON COLUMN employees.manager_id
         IS 'Manager id of the employee; has same domain as manager_id in 
         departments table. Foreign key to employee_id column of employees table.
         (useful for reflexive joins and CONNECT BY query)';</pre>
      <pre>COMMENT ON COLUMN employees.department_id
         IS 'Department id where employee works; foreign key to department_id 
         column of the departments table';</pre>
      <pre>COMMIT;</pre>
      <p class="bodycopy">&nbsp;</p>
    <!-- #EndEditable --></td>
  </tr>
